import customtkinter, CTkMenuBar, CTkTable, tkinter, tkinter.filedialog, tkinter.messagebox, sys, csv, typing, openpyxl, os, My_Organizer_AI_Window, My_Organizer_Interface, locale, warnings
...
class Program(customtkinter.CTk, My_Organizer_Interface.My_Organizer_Interface): 
    ...
    def __edit_line__(self: typing.Self) -> None:  
        ...
        try:
            ...
            if locale.getdefaultlocale()[0] == f"sr_RS": tkinter.messagebox.showerror(f"Greška", f"Nevažeći broj reda ili kolone")
            elif locale.getdefaultlocale()[0] == f"ru_RU": tkinter.messagebox.showerror(f"Ошибка", f"Недопустимый номер строки или столбца")
            else: tkinter.messagebox.showerror(f"Error", f"Invalid row or column number")

    @typing.override
    def __delete_line__(self: typing.Self) -> None: 
        if locale.getdefaultlocale()[0] == f"rs_RS":
            self.rowline: customtkinter.CTkInputDialog = customtkinter.CTkInputDialog(title=f"Red", text=f"Unesite red")
            self.rowline.after(250, lambda: self.rowline.iconbitmap(f"My_Organizer_icon.ico"))
        ...
        try:
            ...
            if self.row_index >= len(self.tablevalue):
                if locale.getdefaultlocale()[0] == f"sr_RS": tkinter.messagebox.showerror(f"Greška", f"Indeks reda van opsega"); return
                elif locale.getdefaultlocale()[0] == f"ru_RU": tkinter.messagebox.showerror(f"Ошибка", f"Индекс строки вне диапазона"); return
            ...
            if locale.getdefaultlocale()[0] == f"sr_RS": tkinter.messagebox.showerror(f"Greška", f"Nevažeći broj reda")
            elif locale.getdefaultlocale()[0] == f"ru_RU": tkinter.messagebox.showerror(f"Ошибка", f"Недопустимый номер строки")
    ...
    def __load_data__(self: typing.Self) -> None: 
        ...
        try:
            ...
            if locale.getdefaultlocale()[0] == f"rs_RS": tkinter.messagebox.showerror(f"Greška", f"Neuspešno učitavanje fajla: {str(e)}")
            elif locale.getdefaultlocale()[0] == f"ru_RU": tkinter.messagebox.showerror(f"Ошибка", f"Не удалось загрузить файл: {str(e)}")
    ...
    def __save_data__(self) -> None: 
        ...
        try:
            ...
            if locale.getdefaultlocale()[0] == f"sr_RS": tkinter.messagebox.showerror(f"Greška", f"Neuspešno čuvanje fajla: {str(e)}")
            elif locale.getdefaultlocale()[0] == f"ru_RU": tkinter.messagebox.showerror(f"Ошибка", f"Не удалось сохранить файл: {str(e)}")
            else: tkinter.messagebox.showerror(f"Error", f"Failed to save file: {str(e)}")

    @typing.override
    def __clean_table__(self: typing.Self) -> None:
        if locale.getdefaultlocale()[0] == f"rs_RS": self.tablevalue: list[[str]] = [[f"Ime i prezime", f"Menadžer", f"Odeljenje", f"Radno mesto", f"Radno vreme", f"Dodatni komentar"]]
        elif locale.getdefaultlocale()[0] == f"ru_RU": self.tablevalue: list[[str]] = [[f"Имя и фамилия", f"Менаджер", f"Отдел", f"Должность", f"Рабочие часы", f"Дополнительный комментарий"]]
